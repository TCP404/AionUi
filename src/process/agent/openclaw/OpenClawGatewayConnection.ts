    ...
    this.request<HelloOk>('connect', params)
      ...
      .catch((err) => {
        ...
        const isPairingRequired =
          ...
          (err instanceof Error && err.message.includes('pairing required'));
      ...
      switch (parsed.type) {
        ...
        case 'res': {
          ...
          if (res.ok) {
            ...
            const errDetails =
              typeof errShape?.details === 'object' && errShape?.details !== null
                ? (errShape.details as Record<string, unknown>)
                : undefined;
    ...
    if (this.pairingPending) {
      ...
      if (this.reconnectAttempts > this.maxPairingRetries) {
        ...
        this.opts.onConnectError?.(
          new Error('Pairing approval timeout: device was not approved within the expected window')
        );
        return;
      }
      if (this.tickTimer) {
      ...
      console.log(
        `[OpenClawGateway] Pairing pending, retry ${this.reconnectAttempts}/${this.maxPairingRetries} in ${this.pairingRetryIntervalMs}ms`
      );
